...
def move_toward(start, target, grid):
    ...
    tr, tc = target
    moves = []
    if r < tr: moves.append(("down", [r+1, c]))
    if r > tr: moves.append(("up", [r-1, c]))
    if c < tc: moves.append(("right", [r, c+1]))
    if c > tc: moves.append(("left", [r, c-1]))
...
Refine the player profile based on their last move."""
    ...
    def _base_behavior_fallback(self, enemy_state: dict, player_state: dict, board_state: dict) -> EnemyAction:
        ...
        if behavior in ("rush_melee", "tank_melee", "swarm_melee", "slow_tank"):
            if dist == 1:
                return EnemyAction(
                    enemy_id=enemy_id, action_type="attack", direction=None,
                    target_position=p_pos, damage=max(1, enemy_state.get("attack", 0) - 0),
                    reasoning="Attacking adjacent player", updated_tactics={}
                )
            else:
                direction = move_toward(e_pos, p_pos, grid)
                if direction != "wait":
                    return EnemyAction(enemy_id=enemy_id, action_type="move", direction=direction,
                                       target_position=None, damage=None, reasoning="Moving toward player", updated_tactics={})
            ...
            if dist <= 2:
                if dist == 1 and behavior == "flee_then_attack":
                    ...
                    if direction != "wait":
                        return EnemyAction(enemy_id=enemy_id, action_type="move", direction=direction,
                                           target_position=None, damage=None, reasoning="Fleeing from player", updated_tactics={})
                return EnemyAction(
                    enemy_id=enemy_id, action_type="attack", direction=None,
                    target_position=p_pos, damage=max(1, enemy_state.get("attack", 0) - 0),
                    reasoning="Ranged attack on player", updated_tactics={}
                )
            else:
                direction = move_toward(e_pos, p_pos, grid)
                if direction != "wait":
                    return EnemyAction(enemy_id=enemy_id, action_type="move", direction=direction,
                                       target_position=None, damage=None, reasoning="Moving to range", updated_tactics={})
            ...
            if dist == 1:
                return EnemyAction(
                    enemy_id=enemy_id, action_type="attack", direction=None,
                    target_position=p_pos, damage=max(1, enemy_state.get("attack", 0) - 0),
                    reasoning="Hit and run attack", updated_tactics={}
                )
            else:
                direction = move_toward(e_pos, p_pos, grid)
                if direction != "wait":
                    return EnemyAction(enemy_id=enemy_id, action_type="move", direction=direction,
                                       target_position=None, damage=None, reasoning="Closing in", updated_tactics={})

        elif behavior == "heals_nearby_enemies":
            direction = move_toward(e_pos, p_pos, grid)
            if direction != "wait":
                return EnemyAction(enemy_id=enemy_id, action_type="move", direction=direction,
                                   target_position=None, damage=None, reasoning="Moving", updated_tactics={})

        return EnemyAction(enemy_id=enemy_id, action_type="wait", direction=None,
                           target_position=None, damage=None, reasoning="Waiting", updated_tactics={})
    ...
    async def run(self, context: dict) -> EnemyAction:
        ...
        try:
            if redis_client:
                ...
                if cached:
                    ...
                    if action:
                        self.log_trace("Cache hit", "check_cache", {"key": cache_key}, {"hit": True}, "Using cached action")
                        return action
        except Exception:
            pass
        ...
        def get_valid_moves(pos, board_state):
            ...
            r, c = pos
            moves = []
            if r > 0 and grid[r-1][c] in (1,3): moves.append("up")
            if r < len(grid)-1 and grid[r+1][c] in (1,3): moves.append("down")
            if c > 0 and grid[r][c-1] in (1,3): moves.append("left")
            if c < len(grid[0])-1 and grid[r][c+1] in (1,3): moves.append("right")
            return moves
...
Keep reasoning under 80 characters — it shows in the game's live AI panel."""
        ...
        try:
            self.log_trace(
                ...
                tool_input={"enemy_pos": enemy_state.get("position"), "player_pos": player_state.get("position")},
                tool_output={},
            ...
            if err:
                action = self._base_behavior_fallback(enemy_state, player_state, board_state)
            else:
        ...
        try:
            if redis_client:
                ...
                if action.updated_tactics:
                    await redis_client.setex(tactics_key, 3600, json.dumps(action.updated_tactics))
