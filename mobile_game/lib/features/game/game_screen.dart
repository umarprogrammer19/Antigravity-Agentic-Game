...
class _GameScreenState extends ConsumerState<GameScreen> {
  ...
  Future<void> _handleFloorCleared(SessionModel session, GameState gameState) async {
    ...
    try {
      ...
      if (mounted) setState(() => _narrativeText = 'Floor Cleared!');
  ...
  Widget build(BuildContext context) {
    ...
    // Loading screen — session/level not ready yet
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // 3. D-pad pinned to bottom-right, above the AI panel
          Positioned(
            bottom: 100,
            right: 16,
            child: DPadControls(
              onDirectionTap: (direction) => _dungeonGame?.handlePlayerMove(direction),
            ),
          ),

          // 4. AI Decision Panel — DraggableScrollableSheet anchors to bottom
