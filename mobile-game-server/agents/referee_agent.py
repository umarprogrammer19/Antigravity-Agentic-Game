...
XP for kill = floor(enemy.max_hp / 5)"""
    ...
    def _validate_move(self, action: dict, player_state: dict, board_state: dict) -> ActionResult:
        ...
        if not grid or not (0 <= r < len(grid) and 0 <= c < len(grid[0])):
            return ActionResult(action_valid=False, invalid_reason="Out of bounds", result_type="invalid", result_narrative="You bump into a wall.", xp_gained=0, enemy_killed=False, floor_cleared=False, session_over=False)
            
        if grid[r][c] == 0:
            return ActionResult(action_valid=False, invalid_reason="Wall", result_type="invalid", result_narrative="You bump into a wall.", xp_gained=0, enemy_killed=False, floor_cleared=False, session_over=False)
            
        if grid[r][c] == 2:
            return ActionResult(action_valid=True, invalid_reason=None, result_type="won", new_player_position=new_pos, damage_dealt=0, damage_taken=0, enemy_killed=False, xp_gained=0, floor_cleared=True, session_over=False, result_narrative="You reach the exit!")
        ...
        if new_pos in all_enemies:
            ...
            return ActionResult(action_valid=False, invalid_reason="Enemy collision during move (should be attack)", result_type="invalid", result_narrative="An enemy is there.", xp_gained=0, enemy_killed=False, floor_cleared=False, session_over=False)

        return ActionResult(action_valid=True, invalid_reason=None, result_type="moved", new_player_position=new_pos, damage_dealt=0, damage_taken=0, enemy_killed=False, xp_gained=0, floor_cleared=False, session_over=False, result_narrative="You move forward.")

    def _validate_attack(self, action: dict, player_state: dict, board_state: dict) -> ActionResult:
        ...
        return None # Let Gemini handle if we don't have exact enemy stats in pure python

    def _validate_special(self, action: dict, player_state: dict, board_state: dict) -> ActionResult:
        return None # Let Gemini handle special actions

    def _validate_wait(self, action: dict, player_state: dict, board_state: dict) -> ActionResult:
        return ActionResult(action_valid=True, invalid_reason=None, result_type="wait", new_player_position=player_state.get("position"), damage_dealt=0, damage_taken=0, enemy_killed=False, xp_gained=0, floor_cleared=False, session_over=False, result_narrative="You wait.")
    ...
    async def run(self, context: dict) -> ActionResult:
        ...
        if res is not None and getattr(res, "action_valid", False):
            self.log_trace(reasoning=f"Player action '{a_type}' validated via rules", tool_called="validate_action", tool_input=action, tool_output=res.model_dump(), decision="Pure Python validation")
            return res
...
Output the ActionResult JSON now."""
        ...
        try:
            self.log_trace(reasoning="Evaluating complex action", tool_called="gemini_eval", tool_input={"action": action}, tool_output={}, decision="Calling Gemini")
            ...
            fallback = ActionResult(action_valid=False, invalid_reason="Action validation failed", result_type="invalid", result_narrative="The action fails.", xp_gained=0, enemy_killed=False, floor_cleared=False, session_over=False)
            self.log_trace(reasoning="AI validation failed", tool_called="fallback_eval", tool_input={}, tool_output={}, decision="Returning invalid action")
            return fallback
