        ...
        try:
            if redis_client:
                ...
                if cached:
                    ...
                    if resp:
                        self.log_trace("Cache hit", "check_cache", {"key": cache_key}, {"hit": True}, "Using cached narrative")
                        return resp
        except Exception:
            pass
        ...
        try:
            ...
            if event_type in NARRATIVE_FALLBACKS:
                if isinstance(NARRATIVE_FALLBACKS[event_type], dict):
                    ...
                    if "{n}" in text: text = text.replace("{n}", str(floor_number + 1))
                else:
                    text = NARRATIVE_FALLBACKS[event_type]
            narrative = NarrativeResponse(event_type=event_type, text=text, display_duration=2000)
